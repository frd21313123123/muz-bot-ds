...
module.exports = {
  ...
  async execute(interaction, client) {
    debugLog(
      `[play] command from guild=${interaction.guildId} user=${interaction.user?.id} query="${interaction.options.getString('query', false) || ''}"`,
    );
    ...
    const existingQueue = client.queues.get(interaction.guildId);
    let queue = existingQueue;
    const queueWasCreated = !queue;
    ...
    // Оптимизация: подключаемся к voice параллельно с resolveQuery,
    // чтобы скрыть часть latency при /play.
    const shouldJoin = !queue.connection || queue.voiceChannel?.id !== voiceChannel.id;
    const joinPromise = shouldJoin ? queue.join(voiceChannel) : null;
    if (joinPromise) debugLog('[play] queue.join started in parallel');
    ...
    try {
      ...
      if (queueWasCreated) {
        if (joinPromise) {
          void joinPromise.then(() => queue.stop()).catch(() => {});
        } else {
    ...
    if (joinPromise) {
      try {
        debugLog('[play] awaiting parallel join');
        await joinPromise;
